...
`BiwtInput` has one field beyond the domain: **`host_cell_type_names`**, the cell types your app
already defines, so imported types can line up with them instead of duplicating them. At the
[rename step](../guide/rename-cell-types.md) BIWT offers a match as placeholder text — a
...
| `cell_template_paths` | TOML files of cell templates. BIWT ships none, so these seed the library offered at the [cell parameters step](../guide/cell-parameters.md) — listed on the landing screen, where the user can add more or drop yours. See [templates and name matching](templates-and-matching.md). |
