...
BIWT is a standalone, pip-installable package. The core install reads `.csv`; `.h5ad` and
`.rds` support are optional extras, because each pulls in a substantially larger dependency
stack.
...
| `pip install "biwt[seurat]"` | `.rds` / `.rda` / `.rdata` (Seurat / SingleCellExperiment) import — **also needs R, see below** |
| `pip install "biwt[gui]"` | The PyQt5 walkthrough UI |
| `pip install "biwt[all]"` | Everything above |

Python 3.9 or newer is required.

Host applications that embed BIWT (such as
[PhysiCell Studio](https://github.com/PhysiCell-Tools/PhysiCell-Studio)) usually supply the
GUI dependencies themselves; check the host's own documentation for which extras it expects.
...
Reading `.rds` / `.rda` / `.rdata` additionally needs a working R with the `Seurat` and
`SingleCellExperiment` R packages, reached through `rpy2`. The whole R stack — the R
interpreter, both R packages, and `rpy2` — installs from conda as **prebuilt binaries**, so
it is fast (no source compile) and lands in the environment's own R, with no dependency on a
system-wide R install.

Replace `<env>` below with the name of the conda environment you are installing into.
...
Step 2 is in its own block on purpose: `conda install` prompts `y/n`, and if it is pasted
together with the following commands the next line is swallowed as the answer to that prompt.
...
> `conda env config vars` pins it to this environment's own R — scoped to the environment,
> re-applied on every activation, with no global PATH changes.
>
> The double quotes are load-bearing: your shell expands `$CONDA_PREFIX` before conda stores
> the value, so conda records an absolute path. That means the value does not follow the
> environment — re-run step 4 after moving, renaming, or cloning the environment.
...
**A common root cause is `rpy2` using the wrong R.** `rpy2` picks its R from `$R_HOME`, and
only falls back to the first `R` on `$PATH` when that is unset. On macOS another R
installation is often reachable earlier on `PATH` (for example, by a symlink in
...
- **Symptom:** `Error in substring(x, m + 1L) : invalid substring arguments` immediately
  followed by `Segmentation fault`, crashing the host application.
