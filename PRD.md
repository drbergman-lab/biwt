...
- The dialog edits the domain in the **host's units** (`preferred_domain.units`, e.g. `micron`) and exposes a data-unit→host-unit **scale factor**:
  - A **`{host-unit}/data unit`** field (e.g. `micron/data unit`) — ratio notation, so the denominator stays singular whatever the host unit is — seeded from `BiwtData.microns_per_data_unit` (Visium). A reset (↺) button restores the file value and is enabled whenever the field differs from it, **including when the field is empty**.
  ...
  - **One axis table (`_DOMAIN_AXES`) is the single source of truth** for the layout, the extent derivation, and the bounds validation. Previously the bounds and the extents were two independent flat lists and only a tuple inside the extent table knew that `width` spanned `xmin`…`xmax`.
