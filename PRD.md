...
- An `obsm` entry counts as coordinates only when it is a 2-D array of exactly 2 or 3 columns (`_find_spatial_key`). The name alone is not enough: an `(N, N)` `spatial_connectivities` matrix matches "spatial" and its first three columns would otherwise be read as x/y/z, outranking real `imagerow`/`imagecol` columns in obs. A candidate of any other width is skipped and the search continues. With two or three cells a square matrix is indistinguishable from coordinates; that is an accepted limit of a shape check.
- The y-flip belongs to `imagerow`/`imagecol` and fires wherever those columns are read. An `obsm` array has no column names, so its numbers are taken as given — a file offering both routes is therefore not expected to agree with itself.
- A probability column qualifies on its name plus any surviving mass once values outside `[0, inf)` are zeroed (`clamp_probabilities`). NaN, ±inf and negatives are not weights, but nor are they a reason to discard the cell type they belong to. The clamp is applied where the per-spot weights are built as well as where the columns are chosen: a raw NaN wins `argmax`, so an unclamped spot would nominate its own type as that spot's maximum.
- An `.rda`/`.rdata` workspace is searched for the object of a supported class rather than the alphabetically first (`base::ls()` returns sorted names). Exactly one match is used; none or several raises a `LoadError` naming what was found and how to re-save, because which one the user meant is unknowable and a single-object file keeps the run reproducible from the file alone. The extension is matched lowercased, so `.RData` works.
...
- **`BiwtInput` is the host's state and nothing else**: `preferred_domain` and `host_cell_type_names`, both re-read at every run. Its name, its template library and its name-matching rule are `create_biwt_widget` arguments — none of them changes between runs, and the library is one the user edits, so re-asking per run would either be pointless or undo their work. The split also means no field can be set on a later resolution and silently ignored, which is what a construction-only field on a per-run value would be.
- The widget holds them for its lifetime and step windows read them from it (`walkthrough.host_name`, `walkthrough.name_matcher`); the session carries run state only. Copying them into every session would put the same setting in two places and invite the two to disagree. `core.cell_types.resolve_name_matcher` does the "host's predicate, else the default bound to the cutoff" resolution — pure, testable without Qt, and called once at construction so every match the widget ever scores is scored the same way.
...
- The dialog edits the domain in the **host's units** (`preferred_domain.units`, e.g. `micron`) and exposes a data-unit→host-unit **scale factor**:
  ...
  - **"Use Data Domain"**: fills data-units = raw data bounds and host-units = raw × factor (or, with no factor, host-units = raw). **"Use {host} Domain"**: fills host-units = the host bounds verbatim. Z is scaled with x and y when the file supplies a z coordinate, and left in host units when it does not — a synthesized ±10 slab is not a measurement in data units, so the factor has nothing to convert (`core.domain.data_has_z`).
  ...
  - **One axis table (`DomainEditorDialog._AXES`) is the single source of truth** for the layout, the extent derivation, and the bounds validation, so a bound and its extent cannot disagree about which pair of numbers they span.
...
- [x] Axis-major grid: each extent shares a grid row with its own axis' bounds (asserted, so it cannot silently regress); each axis gets its own row; `_XY` is derived from `DomainEditorDialog._AXES` rather than spelled out.
...
**Acceptance criteria:**
- [x] No templates ship with the package; the landing screen's list (seeded by `cell_template_paths`) and the runtime loader are the only sources.
- [x] A file listed on the landing screen reaches the step and survives a re-import; one that cannot be read is reported at the step, once, and the step still builds.
- [x] The host's files are listed before the first import, and one the user removes there is gone for every later run.
- [x] `BiwtInput` carries no construction-time setting: `cell_template_paths` is a `create_biwt_widget` argument, so nothing the user edits is re-read per run.
...
- If `cell_templates` is non-empty, Studio assembles a PhysiCell config from the template contents and offers to save it. Studio also ships the PhysiCell template library and passes it to `create_biwt_widget` as `cell_template_paths`.
...
**Behavioral specification:**
- The decision belongs to the host: `create_biwt_widget(name_matches=…)` takes a `Callable[[str, str], bool]`, and supplying it replaces BIWT's rule **and** `BiwtInput.name_match_cutoff` entirely.
