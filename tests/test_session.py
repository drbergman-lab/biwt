...
- toy_seurat.rds / toy_sce.rds / toy_spatialexperiment.rds
    Tiny (10-cell) R objects generated by tests/fixtures/make_fixtures.R.
    Not committed — CI regenerates them on every run so they match whatever
    R/Bioconductor versions conda resolved. Run the script locally (inside an
    env with Seurat + SingleCellExperiment + SpatialExperiment) before running
    the R import tests, or they will skip on the missing files.
...
SCE_RDS = str(FIXTURES / "toy_sce.rds")
SPATIAL_EXPERIMENT_RDS = str(FIXTURES / "toy_spatialexperiment.rds")
ANN_DATA = str(FIXTURES / "test_AnnData.h5ad")

# Cells in the toy R fixtures (see tests/fixtures/make_fixtures.R).
TOY_N_CELLS = 10
...
class TestDataLoader:
    ...
    def test_load_seurat(self):
        ...
        if not os.path.exists(SEURAT_RDS):
            pytest.skip("run tests/fixtures/make_fixtures.R to generate toy R fixtures")
        data = data_loader.load(SEURAT_RDS)
        assert data.n_cells == TOY_N_CELLS
        assert not data.has_spatial

    def test_load_sce(self):
        pytest.importorskip("rpy2")
        pytest.importorskip("anndata2ri")
        if not os.path.exists(SCE_RDS):
            pytest.skip("run tests/fixtures/make_fixtures.R to generate toy R fixtures")
        data = data_loader.load(SCE_RDS)
        assert data.n_cells == TOY_N_CELLS
        assert not data.has_spatial

    def test_load_spatial_experiment(self):
        pytest.importorskip("rpy2")
        pytest.importorskip("anndata2ri")
        if not os.path.exists(SPATIAL_EXPERIMENT_RDS):
            pytest.skip("run tests/fixtures/make_fixtures.R to generate toy R fixtures")
        data = data_loader.load(SPATIAL_EXPERIMENT_RDS)
        assert data.n_cells == TOY_N_CELLS
